...
bool checkShieldCollision(Shields *shields, double shotX, double shotY) {
  ...
  for (unsigned n = 0; n < shields->nb; n++) {
    ...
    if (fabs(shotXScreen - shieldCenterX) < halfHitW) {
      ...
      double relativeX = (shotXScreen - (shieldCenterX - halfHitW)) / shieldWidthRatio;
      ...
      // DEBUG: I'M FED UP I WANT TO UNDERSTAND
      // Edit: I think I understood ^^
      printf("Hit shield %u at block (%d, %d), shotX=%.3f, shotY=%.3f\n", 
             n, blockX, blockY, shotX, shotY);
      
      if (blockX >= 0 && blockX < SHIELD_WIDTH && blockY >= 0 && blockY < SHIELD_HEIGHT) {
