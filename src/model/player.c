...
void playerFire(Player *player, Aliens *aliens) {
  ...
  incrementShootCounter(aliens);
}

void updatePlayerShoot(Player *player, Shields *shields) {
  if (player->shootX != -1 && player->shootY >= 0) {
...
int resolvePlayerHit(Player *player, Aliens *aliens) {
  ...
  for (int i = 0; i < MAX_ALIEN_SHOOTS; i++) {
    ...
    const double shootX = aliens->alienShootX[i] * GAME_WIDTH_RATIO + margin;
    const double shootY = aliens->alienShootY[i];

    if (fabs(shootX - playerX) < halfHitW &&
        fabs(shootY - playerY) < halfHitH) {
